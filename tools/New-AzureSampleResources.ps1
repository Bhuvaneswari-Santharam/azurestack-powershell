...
if (!$ExistingServicePrincipalDetails)
{
    if ($adfs)
    {
        ...
        $azureCertSpInfo = New-AzureCertSp -ApplicationName $azureCertSpName -CertificateName $certName -AzureAdminCredential $credential
    }
    else 
    {
        $azureAppSpInfo = New-AzureAppSp -ServicePrincipalName $azureAppSpName
        $azureCertSpInfo = New-AzureCertSp -ServicePrincipalName $azureCertSpName -CertificateName $certName
    }
}
else
{
